-- Ensure manually submitted SOP reviews are visible to the selected review team.
-- Older submissions used document_teams but left documents.review_team_id null.

update public.documents d
set review_team_id = dt.team_id,
    updated_at = now()
from public.document_teams dt,
     public.source_spaces s
where d.id = dt.document_id
  and s.id = d.source_space_id
  and d.review_team_id is null
  and s.provider = 'tdsource-sop-upload'
  and d.workflow_state = 'awaiting_review';
...
begin
  ...
  insert into public.documents(organization_id, source_space_id, review_team_id, workflow_state)
    values (p_organization_id, v_space_id, p_team_id, 'awaiting_review')
