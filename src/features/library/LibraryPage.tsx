...
import { Archive, Building2, ChevronDown, FileText, Filter, LoaderCircle, Pencil, Save, Search, Star, X } from "lucide-react";
...
const cardLabel = (document: Pick<KnowledgeDocumentSummary, "contentType" | "label">) => document.contentType === "sop" ? "SOP" : labelText(document.label);

export function LibraryPage({ favoritesOnly = false }: { favoritesOnly?: boolean }) {
  const [searchParams] = useSearchParams(); const requestedQuery = searchParams.get("q") || ""; const [query, setQuery] = useState(requestedQuery); const [typeFilter, setTypeFilter] = useState<LibraryTypeFilter>("all"); const [labelFilter, setLabelFilter] = useState<LibraryLabelFilter>("all"); const [sortNewest, setSortNewest] = useState(true);
  ...
  const visibleDocuments = useMemo(() => documents.filter((document) => !favoritesOnly || favoriteIds.has(document.id)).filter((document) => typeFilter === "all" || document.contentType === typeFilter).filter((document) => labelFilter === "all" || document.label === labelFilter).filter((document) => `${document.title} ${document.summary}`.toLowerCase().includes(query.toLowerCase())).sort((a, b) => sortNewest ? Date.parse(b.updatedAt) - Date.parse(a.updatedAt) : Date.parse(a.updatedAt) - Date.parse(b.updatedAt)), [documents, favoriteIds, favoritesOnly, labelFilter, query, sortNewest, typeFilter]);
  ...
  return <div className="page library-page"><header className="page-heading"><div><span className="eyebrow">Approved, access-controlled guidance</span><h1>{favoritesOnly ? "Favorites" : "Knowledge library"}</h1><p>Open and search approved answers available through your teams.</p></div></header>
    <nav className="library-type-tabs" aria-label="Filter knowledge by type">{([['all', 'All'], ['sop', 'SOPs'], ['question', 'Questions']] as const).map(([value, text]) => <button key={value} type="button" className={typeFilter === value ? "is-active" : ""} aria-pressed={typeFilter === value} onClick={() => setTypeFilter(value)}>{text}</button>)}</nav>
