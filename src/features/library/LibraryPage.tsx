...
import { Archive, Building2, ChevronDown, FileText, Filter, LoaderCircle, Pencil, Save, Search, Star, Trash2, X } from "lucide-react";
...
export function LibraryPage({ favoritesOnly = false }: { favoritesOnly?: boolean }) {
  ...
  const [editSourceContent, setEditSourceContent] = useState<SopContent | null>(null); const [page, setPage] = useState(1);
  ...
  const refreshLifecycle = () => { invalidateApiCache("/api/library"); setRevision((value) => value + 1); setSelected(null); setDetail(null); setEdit(null); setEditSourceContent(null); setEditing(false); };
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      ...
      setSelected(null); setDetail(null); setEdit(null); setEditSourceContent(null); setEditing(false); setMessage("");
    };
    document.addEventListener("keydown", closeOnEscape);
    return () => document.removeEventListener("keydown", closeOnEscape);
  }, [selected]);
  const close = () => { setSelected(null); setDetail(null); setEdit(null); setEditSourceContent(null); setEditing(false); setMessage(""); };
  const open = async (document: KnowledgeDocumentSummary) => {
    setSelected(document); setDetail(null); setEdit(null); setEditSourceContent(null); setEditing(false); setMessage("");
    try {
      const loaded = await getJsonCached<LibraryDetail>(`/api/library/${document.id}`, 0);
      setDetail(loaded); setEdit({ ...loaded.draft, steps: [...loaded.draft.steps], warnings: [...loaded.draft.warnings] }); setEditSourceContent(loaded.sourceContent ? structuredClone(loaded.sourceContent) : null); setEditLabel(loaded.label === "verified" ? "verified" : "unresolved");
  ...
  const savePublished = async () => {
    ...
    try {
      const response = await fetch(`/api/library/${selected.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...edit, label: editLabel, sourceContent: editSourceContent }) });
      ...
      setDetail((current) => current ? { ...current, label: editLabel, updatedAt, draft: { ...edit, steps: [...edit.steps], warnings: [...edit.warnings] }, sourceContent: editSourceContent } : current);
  ...
  const pageDocuments = visibleDocuments.slice((page - 1) * 20, page * 20);
  useEffect(() => setPage(1), [favoritesOnly, includeHistorical, labelFilter, query, sortNewest]);
