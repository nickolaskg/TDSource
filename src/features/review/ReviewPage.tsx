...
interface ReviewDetail { id: string; draft: Draft; organizationWide: boolean; sourceMessages: Array<{ provider_message_id: string; author_display_name: string; source_markdown: string; sent_at: string }> }
...
export function ReviewPage() {
  ...
  const dirty = Boolean(detail && edit && JSON.stringify(detail.draft) !== JSON.stringify(edit));
  const saveDraft = async (): Promise<boolean> => {
    if (!selected || !edit) return false;
    setSaving(true); setSaveMessage("");
    try {
      const response = await fetch(`/api/reviews/${selected.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(edit) });
      ...
      setDetail((current) => current ? { ...current, draft: { ...edit, steps: [...edit.steps], warnings: [...edit.warnings] } } : current);
  ...
  return <div className="page review-page"><header className="page-heading"><div><span className="eyebrow">Moderator workspace</span><h1>Review queue</h1><p>Compare, edit, and approve every generated draft against its original conversation thread.</p></div></header>
    <div className="review-banner"><ShieldCheck size={20} /><p><strong>Approval is required.</strong> AI drafts remain unpublished until a Moderator or Admin approves them.</p></div>
