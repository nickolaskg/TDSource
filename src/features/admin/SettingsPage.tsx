...
export function SettingsPage() {
  ...
  const section = (sections.some(({ id }) => id === params.get("section")) ? params.get("section") : "profile") as SettingsSection;
  ...
  return (
    <div className="page settings-page">
      ...
      <div className="settings-layout">
        <nav className="settings-tabs" aria-label="Settings sections">{sections.map(({ id, label, icon: Icon }) => <button key={id} className={section === id ? "active" : ""} onClick={() => chooseSection(id)}><Icon size={17} />{label}</button>)}</nav>
